...
// api/psu-report.ts
var VALID_PSU_IDS = /* @__PURE__ */ new Set([
...
var VALID_FAILURE_MODES = /* @__PURE__ */ new Set([
  "no-power",
  "random-shutdowns",
  "component-damage",
  "noise-sparks",
...
async function hashIp(ip, psuId) {
...
var onRequestPost = /* @__PURE__ */ __name(async ({ request, env: env2 }) => {
  ...
  const notes = rawNotes ? rawNotes.replace(/<[^>]+>/g, "").replace(/[<>'"]/g, "").slice(0, 500) || null : null;
  const ip = request.headers.get("CF-Connecting-IP") ?? "0.0.0.0";
  const ipHash = await hashIp(ip, psuId);
  const windowStart = Math.floor(Date.now() / 1e3) - 86400;
  const rateRow = await env2.PSU_DB.prepare("SELECT COUNT(*) AS cnt FROM psu_reports WHERE ip_hash = ? AND created_at > ?").bind(ipHash, windowStart).first();
  if ((rateRow?.cnt ?? 0) >= 5) {
    return new Response(
  ...
  await env2.PSU_DB.prepare(
    `INSERT INTO psu_reports (psu_id, age_years, status, failure_mode, notes, ip_hash)
       VALUES (?, ?, ?, ?, ?, ?)`
  ).bind(psuId, ageYears, status, failureMode, notes, ipHash).run();
  return new Response(JSON.stringify({ success: true }), {
    status: 200,
    headers: corsHeaders(origin)
  });
...
// api/contact.ts
var VALID_SUBJECTS = /* @__PURE__ */ new Set([
  "data-correction",
  "missing-component",
  "methodology",
  "partnership",
...
function sanitize(str) {
  return str.trim().replace(/<[^>]*>/g, "").slice(0, 5e3);
}
...
var onRequest = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    ...
    const clientIP = context2.request.headers.get("CF-Connecting-IP") || "unknown";
...
// ../.wrangler/tmp/pages-yc6Enz/functionsRoutes-0.9209217050254015.mjs
var routes = [
  {
